...
export const api = {
  ...
  // Phone Sales (Supabase REST - kalıcı depolama)
  async getPhoneSales(): Promise<PhoneSale[]> {
    const result = await fetchSupabase("/phone_sales?select=*&order=created_at.desc");
  ...
  async addPhoneSale(phoneSale: Omit<PhoneSale, "id">): Promise<PhoneSale> {
    const payload = {
      brand: phoneSale.brand,
      model: phoneSale.model,
      imei: phoneSale.imei || '',
      purchase_price: phoneSale.purchasePrice,
      sale_price: phoneSale.salePrice,
      profit: phoneSale.profit,
      customer_name: phoneSale.customerName || '',
      customer_phone: phoneSale.customerPhone || '',
      notes: phoneSale.notes || '',
      date: phoneSale.date,
      payment_method: phoneSale.paymentMethod || null,
      payment_details: phoneSale.paymentDetails || null,
    };
    const result = await fetchSupabase("/phone_sales", {
      method: "POST",
  ...
  async updatePhoneSale(id: string, phoneSale: PhoneSale): Promise<PhoneSale> {
    ...
    const result = await fetchSupabase(`/phone_sales?id=eq.${id}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
    const row = Array.isArray(result.data) ? result.data[0] : result.data;
    return { ...phoneSale, id: row.id };
  },

  async deletePhoneSale(id: string): Promise<void> {
    await fetchSupabase(`/phone_sales?id=eq.${id}`, {
      method: "DELETE",
    });
  },

  // Phone Stocks (Supabase REST - kalıcı depolama)
  ...
  async addPhoneStock(phoneStock: Omit<PhoneStock, "id">): Promise<PhoneStock> {
    const payload = {
      ...
      imei: phoneStock.imei || '',
      purchase_price: phoneStock.purchasePrice,
      sale_price: phoneStock.salePrice,
      notes: phoneStock.notes || '',
      status: phoneStock.status || 'in_stock',
    };
  ...
  async updatePhoneStock(id: string, phoneStock: Partial<PhoneStock>): Promise<PhoneStock> {
    const payload: any = {};
    if (phoneStock.brand !== undefined) payload.brand = phoneStock.brand;
    if (phoneStock.model !== undefined) payload.model = phoneStock.model;
    if (phoneStock.imei !== undefined) payload.imei = phoneStock.imei;
    if (phoneStock.purchasePrice !== undefined) payload.purchase_price = phoneStock.purchasePrice;
    if (phoneStock.salePrice !== undefined) payload.sale_price = phoneStock.salePrice;
    if (phoneStock.notes !== undefined) payload.notes = phoneStock.notes;
    if (phoneStock.status !== undefined) payload.status = phoneStock.status;
