...
// Phone Stocks (Supabase - kalıcı depolama)
...
defineRoute("POST", "/phone-stocks", async (c: Context) => {
    try {
        ...
        const payload = {
            ...
            model: body.model,
            imei: body.imei || '',
            purchase_price: body.purchasePrice ?? body.purchase_price ?? 0,
...
defineRoute("PUT", "/phone-stocks/:id", async (c: Context) => {
    try {
        ...
        if (body.imei !== undefined) payload.imei = body.imei;
        if (body.purchasePrice !== undefined) payload.purchase_price = body.purchasePrice;
...
// Phone Sales (Supabase - kalıcı depolama)
...
defineRoute("POST", "/phone-sales", async (c: Context) => {
    try {
        ...
        const payload = {
            ...
            payment_details: body.paymentDetails ?? body.payment_details ?? null,
        };
        const { data, error } = await supabase
            .from("phone_sales")
            .insert([payload])
...
defineRoute("DELETE", "/phone-sales/:id", async (c: Context) => {
    try {
        const id = c.req.param("id");
        const { error } = await supabase
            .from("phone_sales")
