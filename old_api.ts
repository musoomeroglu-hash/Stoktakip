...
async function fetchSupabase(table: string, options: RequestInit = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Supabase REST Hatas─▒ (${response.status}): ${errorText}`);
    }

    const data = await response.json();
    return { data };
  } catch (error: any) {
    console.error(`ÔØî Supabase REST Error [${table}]:`, error);
    throw error;
  }
}
...
async function fetchAPI(endpoint: string, options: RequestInit = {}) {
  ...
  console.log('­şöÁ API Request:', url);
  ...
  try {
    ...
    console.log(`­şöÁ Response status [${endpoint}]:`, response.status);
    ...
    if (!response.ok) {
      ...
      console.error('ÔØî API Error:', {
        url,
        status: response.status,
        statusText: response.statusText,
        error: errorText
      });
      throw new Error(`API Hatas─▒ (${response.status}): ${errorText || response.statusText}${debugMessage}`);
    }
    ...
    if (error.name === 'AbortError') {
      console.error(`ÔØî API Timeout: ${url} (10s passed)`);
      throw new Error(`─░nternet ba─şlant─▒s─▒ yava┼ş veya sunucu yan─▒t vermiyor (Timeout: ${endpoint})`);
    }
    console.error('ÔØî Fetch Error:', error);
    throw error;
  }
}
...
export interface RepairRecord {
  id: string;
  customerId?: string; // Opsiyonel m├╝┼şteri ba─şlant─▒s─▒
  customerName: string;
...
export interface Customer {
  ...
  phone: string;
  email?: string;
  debt: number; // Bor├ğ (m├╝┼şterinin bize olan borcu)
  credit: number; // Alacak (bizim m├╝┼şteriye olan borcumuz)
  notes?: string;
  createdAt: string;
}
...
export const api = {
  ...
  async addPurchase(purchase: Omit<Purchase, "id" | "created_at">): Promise<Purchase> {
    ...
    if (!isValidUUID(data.supplier_id)) {
      throw new Error(`Kritik Hata: Se├ğilen tedarik├ği ID'si ge├ğersiz (${data.supplier_id}). L├╝tfen tedarik├ğiyi listeden tekrar se├ğin veya yeni bir tedarik├ği olu┼şturun.`);
    }
    ...
    if (items && Array.isArray(items)) {
      for (const item of items) {
        if (!isValidUUID(item.productId)) {
          ...
          throw new Error(`Kritik Hata: "${productName}" ├╝r├╝n├╝ eski bir kay─▒t format─▒nda (ID: ${item.productId}). L├╝tfen bu ├╝r├╝n├╝ silip tekrar ekleyin.`);
        }
      }
    }
